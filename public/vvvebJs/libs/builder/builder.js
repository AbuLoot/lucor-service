				...
				for (i in blocks)
				{
					...
					if (block)
					{
						...
						if (block.image) {
							...
							item.css({
								backgroundImage: "url(" + ((block.image.indexOf('//') == -1) ? '/vvvebJs/libs/builder/':'') + block.image + ")",
								backgroundRepeat: "no-repeat"
							})
						}
		...
		function drawComponentsTree(tree)
		{
			...
			for (i in tree)
			{
				...
				if (tree[i].children.length > 0) {
					...
					<label for="id' + j + '" style="background-image:url(/vvvebJs/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
